...
# choose.clust <- function(){readline("How many clusters to form? ")} 
...
write.table(kclust.out, file="kmeans_out.csv", sep=",", col.names=NA)
dataCSV <- read.table(file='kmeans_out.csv', sep=',', header=T, row.names=NULL)
colnames(dataCSV)[1] <- "CallingNumber"
dataCSVJSON <- toJSON(dataCSV)
View(dataCSVJSON)
